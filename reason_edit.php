...
$PAGE->set_url(new moodle_url('/local/monlaututoria/reason_edit.php', ['id' => $id]));
...
$service = new \local_monlaututoria\service\catalogue_service(
    \local_monlaututoria\service\catalogue_service::TYPE_REASON,
    $repository
);
...
$title = $id
    ? get_string('reason_edit', 'local_monlaututoria')
    : get_string('reason_create', 'local_monlaututoria');
$PAGE->set_title($title);
$PAGE->set_heading($title);

$form = new \local_monlaututoria\form\reason_form();

if ($form->is_cancelled()) {
    redirect(new moodle_url('/local/monlaututoria/reasons.php'));
