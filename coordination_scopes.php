...
if ($save && confirm_sesskey() && $selecteduserid > 0) {
    ...
    redirect(new moodle_url('/local/monlaututoria/coordination_scopes.php', ['userid' => $selecteduserid]), get_string('coordination_scope_saved', 'local_monlaututoria'));
}
...
$users = $DB->get_records_select('user', 'deleted = 0', null, 'lastname ASC, firstname ASC, id ASC');
...
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('coordination_scopes_title', 'local_monlaututoria'));
echo $renderer->single_select(new moodle_url('/local/monlaututoria/coordination_scopes.php'), 'userid', $useroptions, $selecteduserid, null, 'coordinationscopeuserselector');
...
if ($selecteduserid > 0) {
    ...
    foreach ($cohorts as $cohort) {
        echo html_writer::div(html_writer::checkbox('cohortids[]', (int) $cohort->id, in_array((int) $cohort->id, $currentcohortids, true), format_string($cohort->name)), 'form-check');
    }
    echo html_writer::empty_tag('input', ['type' => 'submit', 'class' => 'btn btn-primary mt-3', 'value' => get_string('coordination_scope_save', 'local_monlaututoria')]);
