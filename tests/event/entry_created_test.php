...
/**
 * Tests for the local_tut_entry events.
 *
 * @package    local_monlaututoria
 * @copyright  2026 Monlau Tutoria Project
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class entry_created_test extends \advanced_testcase {
    ...
    public function test_create_triggers_entry_created(): void {
        $this->resetAfterTest();
        ...
        $matching = array_values(array_filter($events, static fn ($event) => $event instanceof entry_created));
        $this->assertCount(1, $matching);

        $event = $matching[0];
        $this->assertSame($id, $event->objectid);
        $this->assertSame($student->id, $event->relateduserid);
        $this->assertSame(get_admin()->id, $event->userid);
        $this->assertSame($tutor->id, $event->other['tutorid']);
        $this->assertSame($academicyearid, $event->other['academicyearid']);
    }
}
