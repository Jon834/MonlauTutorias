...
use local_monlaututoria\service\followup_service;
use local_monlaututoria\domain\followup_status;
use local_monlaututoria\domain\priority_level;

/**
 * Tests for the local_tut_followup events (phase 6.2/6.3).
 *
 * @package    local_monlaututoria
 * @copyright  2026 Monlau Tutoria Project
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class followup_created_test extends \advanced_testcase {
    ...
    public function test_create_triggers_followup_created(): void {
        $this->resetAfterTest();
        ...
        $matching = array_values(array_filter($events, static fn ($event) => $event instanceof followup_created));
    ...
    public function test_close_with_entry_triggers_followup_updated_with_closingentryid(): void {
        $this->resetAfterTest();
        ...
        $matching = array_values(array_filter($events, static fn ($event) => $event instanceof followup_updated));
        $this->assertCount(1, $matching);
        $this->assertSame(followup_status::COMPLETED, $matching[0]->other['newstatus']);
        $this->assertSame($closingentryid, $matching[0]->other['closingentryid']);
    }
}
