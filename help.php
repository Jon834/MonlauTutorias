...
echo $renderer->page_header_card(
    ...
    get_string('help_page_intro', 'local_monlaututoria'),
    null,
    null,
    [],
...
foreach (['entry', 'agreement', 'followup', 'referral'] as $concept) {
    ...
    $html .= html_writer::tag('p', get_string("help_concept_{$concept}_short", 'local_monlaututoria'));
    $html .= html_writer::tag('p', get_string("help_concept_{$concept}_full", 'local_monlaututoria'), ['class' => 'text-muted']);
...
echo html_writer::tag('p', get_string('help_visibility_body', 'local_monlaututoria'));
